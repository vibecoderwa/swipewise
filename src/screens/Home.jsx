...
export default function HomeScreen({ hasAccounts, insights, error, onConnected }) {
  if (!hasAccounts) {
    return (
      <div className="screen">
        <ScreenHeader eyebrow="Welcome" title="Your wallet, " em="optimized." />
        {error && <div className="error">{error}</div>}
        <div className="card">
          <div className="metric-eyebrow">Get started</div>
          <div className="metric-sub" style={{ marginTop: 6, marginBottom: 16 }}>
            Connect a card. We'll quietly tell you which one to use — and what you've been <b>leaving on the table</b>.
          </div>
  ...
  return (
    <div className="screen">
      <ScreenHeader eyebrow="Reflective · last 30 days" title="What you " em="left on the table" />
      {error && <div className="error">{error}</div>}

      <div className="card">
        <div className="metric-eyebrow">Missed rewards</div>
      ...
      {!user_cards.length && (
        <div className="card">
          ...
          <div className="metric-sub" style={{ marginTop: 6 }}>
            Match your accounts to real cards in <b>Settings</b> so we can compute rewards.
          </div>
        </div>
      )}
